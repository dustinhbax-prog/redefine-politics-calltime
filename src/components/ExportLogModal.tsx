...
export default function ExportLogModal({ onClose }: Props) {
  const [password, setPassword] = useState('')
  const [data, setData] = useState<{ count: number; entries: ExportLogEntry[] } | null>(null)
  ...
  const fetchLog = async () => {
    ...
    setError('')
    try {
      const r = await fetch(`/api/admin/export-log?password=${encodeURIComponent(password)}&limit=500`)
  ...
  const exportCsv = () => {
    if (!data) return
    const headers = ['ID', 'Timestamp', 'IP', 'Type', 'Context', 'Rows']
    const csvRows = data.entries.map(e =>
      [e.id, e.ts, e.ip, e.export_type, e.context, e.row_count]
    ...
    a.download = `export-log-${new Date().toISOString().slice(0, 10)}.csv`
    a.click()
  }
    ...
    >
      ...
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-terminal-border flex-shrink-0">
          <div>
            <span className="text-terminal-accent text-sm font-bold tracking-widest uppercase">Export Log</span>
            {data && (
              <span className="ml-4 text-terminal-muted text-xs">{data.count} exports logged</span>
            )}
          </div>
          ...
          <div className="flex-1 overflow-auto min-h-0">
            <table className="w-full text-xs border-collapse">
              <thead className="sticky top-0 bg-terminal-panel z-10">
                <tr>
                  {['Time (UTC)', 'IP', 'Type', 'Context', 'Rows Exported'].map(h => (
                ...
                {data.entries.map((e, i) => (
                  <tr key={e.id} className={`border-t border-terminal-border hover:bg-white/[0.08] ${i % 2 === 0 ? '' : 'bg-white/[0.04]'}`}>
                    ...
                    <td className="px-3 py-1.5">
                      <span className="px-1.5 py-0.5 border border-terminal-border text-terminal-accent rounded uppercase tracking-wider text-xs">
                        {e.export_type || '—'}
                      </span>
                    </td>
                    <td className="px-3 py-1.5 text-terminal-text max-w-[300px] truncate" title={e.context}>{e.context || '—'}</td>
                    <td className="px-3 py-1.5 text-terminal-green font-bold">{e.row_count ?? '—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
